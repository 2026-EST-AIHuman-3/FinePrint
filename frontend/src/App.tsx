...
export default function App() {
  ...
  const handleAnalyze = async (submission: AnalyzeSubmission) => {
    const { type, serviceName, url, documentType = "terms", file } = submission;
    setIsLoading(true);
    ...
    try {
      ...
      const newItem: HistoryItem = {
        ...
        serviceName: status.service_name || serviceName,
        type,
        query: type === "url" ? url! : type === "document" ? file!.name : serviceName,
        date: new Date().toISOString().split("T")[0],
        queries: [],
        suggestedQuestions: [
          `${status.service_name} 구독을 해지하면 환불받을 수 있나요?`,
          `${status.service_name}의 자동 결제와 갱신 조건이 궁금해요.`,
          `${status.service_name} 약관에서 꼭 확인해야 할 조항은 무엇인가요?`,
        ],
      ...
      setSuccessMessage(`${newItem.serviceName} 약관 준비 완료!`);
