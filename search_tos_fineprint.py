...
def prompt_for_terms_url() -> str | None:
    """자동 탐색 실패 시 사용자가 복사한 이용약관 URL을 받는다."""
    answer = input("자동 탐색에 실패했습니다. 이용약관 페이지의 URL을 복사해서 입력하시겠습니까? (y/n): ").strip().lower()
...
def main() -> int:
    ...
    client = None
    manual_terms_url: str | None = None
    try:
    ...
    if not service_domains:
        manual_terms_url = prompt_for_terms_url()
        if not manual_terms_url:
            print("공식 도메인을 찾지 못해 저장하지 않습니다.")
            return 1
        manual_host = urlparse(manual_terms_url).netloc
        service_domains = (registrable_domain(manual_host),)
        print(f"사용자 제공 이용약관 URL을 확인합니다: {manual_terms_url}")

    print(f"확인된 공식 도메인: {', '.join(service_domains)}")
    if manual_terms_url:
        manual_candidate = Candidate(
            url=manual_terms_url,
    ...
    if client and not manual_terms_url:
    ...
    elif client and manual_terms_url:
        alternate_candidates = search_alternate_terms_candidates(
            client,
            service_name,
            service_domains,
            {manual_terms_url},
    ...
    for candidate in candidates[: max(args.top_k, 0)]:
        ...
        if candidate.detected_type == "terms" and not revision_date and manual_terms_url:
