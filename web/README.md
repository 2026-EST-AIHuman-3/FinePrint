...
Windows에서 프로젝트 상위 경로에 한글이 포함된 경우 `.env`의
`CHROMA_DB_PATH="Siyeong/db"`처럼 프로젝트 루트 기준 ASCII 상대 경로를
사용해야 합니다. 따라서 Uvicorn은 반드시 `FindPrint2` 루트에서 실행하세요.
API 서버는 프로젝트 `.env` 값을 운영체제에 상속된 동명 환경변수보다 우선해
사용하므로, 키를 교체한 뒤 별도로 터미널 환경변수를 지울 필요는 없습니다.
...
- `POST /api/v1/agent/analyze`: `/questions`와 동일한 버전형 Agent API

서비스명 준비 결과의 `service_documents_ready`가 `false`이면 UI가 URL 또는
문서 입력 화면으로 전환합니다.

Agent API는 FastAPI 이벤트 루프를 막지 않도록 LangGraph의 비동기 실행기를
사용합니다. `include_trace`를 켜면 의도 분류 이후 검색, 생성, 검증, 재검색 또는
재생성 라운드의 실행 기록도 함께 확인할 수 있습니다.

```powershell
$body = @{
  service_name = "넷플릭스"
  question = "해지했는데 다음 달에도 결제됐어요. 환불 가능한가요?"
  include_trace = $true
} | ConvertTo-Json
